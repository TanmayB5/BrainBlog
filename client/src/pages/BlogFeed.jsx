...
export default function BlogFeed() {
  ...
  const handleLike = async (blogId) => {
    try {
      await blogAPI.incrementLike(blogId);
      setBlogs((prevBlogs) => prevBlogs.map(blog =>
        (blog.id === blogId || blog._id === blogId)
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="p-6 flex flex-col gap-4">
                        ...
                        <div className="flex items-center gap-4 text-sm text-text-medium mb-2">
                          <span className="flex items-center gap-1">
                          ...
                          <button
                            ...
                            className={`ml-2 px-3 py-1 rounded-full font-semibold transition-colors ${likedBlogs[blogId] ? 'bg-pink-500 text-white' : 'bg-light-beige text-text-dark hover:bg-pink-100'}`}
                            disabled={likedBlogs[blogId]}
                          >
                            {likedBlogs[blogId] ? 'Liked' : 'Like'}
                          </button>
                        </div>
                        {/* Read More Button */}
                        {blogId ? (
